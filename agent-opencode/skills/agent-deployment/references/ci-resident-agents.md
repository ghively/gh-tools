...
9. **Platform lock-in via skills.** A skill authored for Copilot's `.github/copilot/skills/` surface doesn't run anywhere else. Fix: keep the SKILL.md body portable (OpenCode shape), and treat platform-specific wrapper files as adapters. The `opencode-authoring` skill's conversion references cover this.
