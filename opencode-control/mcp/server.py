...
Drives the OpenCode AI coding agent (sst/opencode) two ways:
  ...
  1. HTTP server  — `opencode serve` exposes ~188 REST operations (a legacy API
...
async def oc_discover(query: str = "", limit: int = 60) -> str:
    """Search the live OpenAPI surface (all ~188 operations pulled from the server's
...
async def _cli(args: list, timeout: float = 60) -> tuple:
    ...
    try:
        ...
        proc.kill()
        await proc.wait()  # reap — otherwise the killed child lingers as a zombie
        return "", "timed out"
    ...
    if a == "start":
        ...
        for _ in range(30):
            ...
            if proc.poll() is not None:
                ...
                return (f"opencode serve exited immediately (code {proc.returncode}). "
                        f"Check the binary/port — try `opencode serve --port {port}` by hand.")
            try:
...
async def oc_acp_prompt(prompt: str, cwd: str = "", mode: str = "", model: str = "",
    ...
    if permission not in ("reject", "allow", "always"):
        return (f"Unknown permission '{permission}' — use reject (read-only), "
                f"allow (approve once), or always (approve+remember).")
