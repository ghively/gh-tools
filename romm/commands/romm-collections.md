...
1. **Survey what exists** — `romm_collections(kind="all")`; virtual
   collections via `romm_call("GET", "/api/collections/virtual",
   query_params={"type": "franchise"})` (types: franchise, genre, company,
   mode). Don't duplicate an existing or virtual collection — point the user
   at it instead.
...
3. **Smart collection** (auto-updating, rule-based) — translate the user's
   ...
   ["backlogged"]}'`. Verify the rule by running the same filters through
   `romm_roms` first and showing the match count.
...
5. **Maintenance** — rename/describe via `romm_collection_update(...,
   confirm=True)`; delete via `romm_collection_delete(..., confirm=True)`
   (ROMs are never touched by collection deletes).
