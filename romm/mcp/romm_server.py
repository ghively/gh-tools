...
def romm_roms(
    ...
    Args:
        ...
        order_by: name | fs_size_bytes | created_at | first_release_date ...
        order_dir: asc | desc.
...
@mcp.tool()
@_tool_error
def romm_collections(kind: str = "all") -> str:
    """List collections.

    Args:
        kind: manual | smart | virtual | all. Virtual collections are
            auto-generated (franchises, genres, companies...).
    """
    out: dict[str, Any] = {}
    ...
    if kind in ("virtual", "all"):
        out["virtual_note"] = ("virtual collections require a type filter; "
                               "use romm_call GET /api/collections/virtual "
                               "with query_params={'type': 'franchise'} "
                               "(franchise|genre|company|mode|developer|publisher)")
    return _dump(out)
...
def romm_scan(
    ...
    """Trigger a library scan (the web UI's Scan button).

    RomM only exposes scans over Socket.IO with a session cookie, so this
    needs `username`/`password` in config.local.json (the API key alone
    cannot mint a session — verified against 5.0.0 source and live server).
    ...
    Args:
        ...
        wait_seconds: how long to wait for completion before returning
            (scan continues server-side on timeout).
    ...
    # 2. Connect Socket.IO with that cookie and emit "scan".
    ...
    try:
        ...
        finished = done.wait(timeout=wait_seconds)
    finally:
        try:
            sio.disconnect()
        except Exception:
            pass
        # Best-effort logout of the temporary session.
        try:
    ...
    if finished:
        ...
        out["note"] = (f"scan still running after {wait_seconds}s — it continues "
                       "server-side; check romm_stats / romm_platforms for results")
    if events:
        out["progress_sample"] = events[-20:]
    return _dump(out)
